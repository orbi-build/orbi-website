...
**这一节约束 e2e 验收方（合并后做放行判断的那个人/agent），不是沙箱里的交付方。**
交付方没有带界面浏览器、没有部署凭据，它的门禁是 CI + headless，够了。
**不要把本节要求抄进票面的交付方验收项** —— 沙箱做不到，只会让 PR 卡在证据不足
（2026-09-21 在 #354 / #356 与 cloud 的两张票上同时犯过）。
真实浏览器那部分写「由维护者补齐」。
