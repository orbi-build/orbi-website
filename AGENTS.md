...
它只证明**文件存在且能解码**。一张糊的、变形的、带着视频角标的图，它照样是绿的。

2026-09-21 实测翻车：博文七张 step 配图，我跑完 `broken: 0` 就写了「9 图零破图，
通过」。实际三个问题一个没抓到 —— 分辨率只有需要的 0.82 倍（Retina 上发虚）、
图是从视频里截的帧（带 `7 · SUBSCRIBE` 角标、`YOU` 水印、大片黑边）、
`step-1` 原图 1280×360 被塞进 720 的槽位纵向拉伸近 2 倍（见 #356）。

**每张图四条都要过，缺一条就不算验收：**

| 查什么 | 怎么查 | 通过线 |
|---|---|---|
| 够不够清晰 | `naturalWidth >= rect.width * devicePixelRatio` | 倍率 ≥ 1.0 |
| 有没有变形 | 原图 `naturalWidth/naturalHeight` vs 显示 `rect.width/rect.height` | 两者之差 < 0.02 |
| 一组图是否统一 | 同组各图的原图比例两两比较 | 全部相等 |
| **图里是什么** | **下载原图，Read 它，用眼睛看** | 无视频角标 / 水印 / 黑边 / 错内容 |

最后一条没有自动判据，只能看。**跳过它，等于没验收这张图。**
- 改了布局的 PR，对照改动前的截图看
