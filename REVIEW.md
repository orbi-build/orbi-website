...
**结论：测试要求无空格、爬虫要求有空格，两者直接冲突。** 正解是调整测试解析器使其与浏览器
语义一致，但这超出本次授权范围（测试是团队刻意设计的内容不变量）。已回退，保留原 `<br>`，
问题记录待产品决策。
...
- **P1-2 标题文本粘连**：测试与爬虫需求冲突，见订正 2
- **P1-3 英文页加载中文字体**：审查指出英文页 nav/footer 含「中文」4 个字形，
  移除 webfont 后将回退系统 CJK 字体。仍建议移除，但需确认可接受该细微视觉差异
- **死资源** `flow-en.svg` / `flow-zh.svg` / `where.svg`（9KB，零引用）：
  确认无用后可删，但可能是待用素材
