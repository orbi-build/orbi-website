...
里程碑跑完后，引擎会开一张票，列出候选版本，然后停下来等选择。

版本自动推进仍然锁着。「下一版发什么」不该由发版引擎替人决定。

问题出在回答方式。票里写着：请运行 `orbi milestone set v0.5.40`。这是要在宿主机上执行的 CLI 命令。托管租户没有宿主机，沙箱跑在我们的机器上。
...
我们属于前一类。引擎采用 AGPL-3.0 开源、采用 open source (AGPL-3.0) 并支持自托管。更直接的证据是：同一套引擎已经在我们自己的组织里
...
我们有机会在动手之前把两边都读一遍。这才是建立在采用 AGPL-3.0 开源软件之上的真正奢侈，值那一天。
...
继续阅读：[Cursor 对比](/zh/compare/cursor/) 与 [Cloud](/zh/cloud/)。
